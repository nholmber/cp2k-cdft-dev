...
CMK   INCLUDE 'func.inc'
      PARAMETER (SMALL=1.D-10)
      PARAMETER (PI34= 0.75D0 /3.14159265358979323846264D0,
     +           THIRD=1.D0/3.D0)
C     ==--------------------------------------------------------------==
C..Exchange
      IF(MFXCX.EQ.1) THEN
...
C     ==--------------------------------------------------------------==
      ...
      FX = X*X+B*X+C
      QX = DATAN(Q/(TWO*X+B))
      EVWN=A*(DLOG(RS/FX)+F1*QX-F2*(DLOG((X-X0)**2/FX)+F3*QX))
      TXPB=TWO*X+B
      TTQQ=TXPB*TXPB+Q*Q
...
C     ==--------------------------------------------------------------==
     ...
     *           B3=1.110667363742916D0,B4=0.02359291751427506D0)
      PARAMETER (O3=1.0D0/3.0D0)
C     ==--------------------------------------------------------------==
      TOP=A0+RS*(A1+RS*(A2+RS*A3))
      DTOP=A1+RS*(2.0D0*A2+3.0D0*A3*RS)
      BOT=RS*(B1+RS*(B2+RS*(B3+RS*B4)))
      DBOT=B1+RS*(2.0D0*B2+RS*(3.0D0*B3+RS*4.0D0*B4))
      EC=-TOP/BOT
...
C PERDEW CORRELATION: PRB 33, 8822 (1986)
      ...
      PARAMETER(OB3=1.D0/3.D0, FPI=4.0D0*3.14159265358979323846264D0)
C     ==--------------------------------------------------------------==
      AA    = GRHO
      A     = SQRT(AA)
      BR1   = RHO**OB3
...
cmb-> non i-dependent stuff
      ...
      DO i=0,4
         ...
         gab=gab+tab
         gx=gx+txx
         dgaa_drho=dgaa_drho-f83rho*denaa*taa*FLOAT(i)
         dgab_drho=dgab_drho-f83rho*denab*tab*FLOAT(i)
         dgx_drho=dgx_drho-f83rho*denx*txx*FLOAT(i)
         dgaa_dgr=dgaa_dgr+bygr*denaa*taa*FLOAT(i)
         dgab_dgr=dgab_dgr+bygr*denab*tab*FLOAT(i)
         dgx_dgr=dgx_dgr+bygr*denx*txx*FLOAT(i)
      ENDDO
